...
to black-patches-around
  ask patches [
    ifelse any? patches with [pcolor = black] in-radius 2.1 [
      set neighbors-black other patches with [pcolor = black] in-radius 2
    ][
      set neighbors-black nobody
    ]
  ]
end
...
to side-winder
  ...
  ask turtles [
    ...
    if pxcor = max-pxcor - 1 [set heading 0]
    if pycor = max-pycor - 1 [set heading 90]
...
to coloring
  let initial one-of patches with [pcolor = white]
 ...
 ;   set plabel-color black
  ...
  let i 0
  while [count patches with [pcolor = white and step = -1] > 0][
    ask patches with [step = i] [
      ask neighbors4 with [pcolor = white and step = -1][
        set step i + 1
  ;      set plabel step
 ;       set plabel-color black
      ]
  ...
  let num-d max [step] of patches
  ask patches [
  ask patches with [pcolor = white] [
    set pcolor 69.9 - 9.9 * step / num-d
  ]
  ]
