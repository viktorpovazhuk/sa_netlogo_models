...
to make-turtles
  ...
  create-turtles num-mine [set strategy "mine" set color magenta ]
end
...
to update-history
  ...
  if strategy = "mine" [ mine-history-update ]
end
...
to mine
  set num-mine-games num-mine-games + 1
  let x who
  let y [who] of partner
  let l table:get total-history (list x y)
  let p 0
  ifelse (length l > 0) [
    set p (length (filter [i -> i = "C"] l)) / (length l) * 100
  ] [
    set p 100
  ]
  ifelse random 100 < p [
...
to do-scoring
  ...
  set mine-score  (calc-score "mine" num-mine)
end
...
"mine" 1.0 0 -5825686 true "" "if num-mine-games > 0 [ plot mine-score / (num-mine-games) ]"
