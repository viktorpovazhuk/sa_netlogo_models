...
to set-turtle-counts
  set num-random num-strategy-agents
  set num-cooperate num-strategy-agents
  set num-defect num-strategy-agents
  set num-tit-for-tat num-strategy-agents
  set num-joss num-strategy-agents
  set num-anonymous num-strategy-agents
end
...
to make-turtles
  ...
  create-turtles num-grofman [set strategy "grofman" set color yellow - 1 ]
  create-turtles num-joss [set strategy "joss" set color pink ]
  create-turtles num-anonymous [set strategy "anonymous" set color orange + 1 ]
end
...
to select-action ;;turtle procedure
  ...
  if strategy = "joss" [ joss ]
  if strategy = "anonymous" [ anonymous ]
end
...
to update-history
  ...
  if strategy = "joss" [ joss-history-update ]
  if strategy = "anonymous" [ anonymous-history-update ]
end
...
to anonymous
  set num-anonymous-games num-anonymous-games + 1
  let p 0.3 + random-float 0.4
...
to anonymous-history-update
  ; nothing
end

to joss
  set num-joss-games num-joss-games + 1
  set partner-defected? item ([who] of partner) partner-history
  ifelse partner-defected? [
    set defect-now? true
  ] [
    ifelse random-float 1 < 0.9 [ set defect-now? false ] [ set defect-now? true ]
  ]
end

to joss-history-update
  if partner-defected? [
    set partner-history
...
to grofman
  ...
  let partner-same? item ([who] of partner) partner-history
  ifelse partner-same? [
    set defect-now? false
  ] [
    ifelse random-float 7 < 2 [ set defect-now? false ] [ set defect-now? true ]
  ]
end

to grofman-history-update
  set partner-history
    (replace-item ([who] of partner) partner-history (partner-defected? = defect-now?))
end
...
to tit-for-tat
  ...
  set partner-defected? item ([who] of partner) partner-history ; here partner-defected? set to history value, later in 'go' it is updated
  ifelse (partner-defected?) [
    set defect-now? true
  ] [
    set defect-now? false
...
to do-scoring
  set random-score  (calc-score "random" num-random) ; we count games for each turtle -> average payoff <= 5
  ...
  set joss-score  (calc-score "joss" num-joss)
  set anonymous-score  (calc-score "anonymous" num-anonymous)
end
...
"grofman" 1.0 0 -4079321 true "" "if num-grofman-games > 0 [ plot grofman-score / (num-grofman-games) ]"
"joss" 1.0 0 -2064490 true "" "if num-joss-games > 0 [ plot joss-score / (num-joss-games) ]"
"anonymous" 1.0 0 -817084 true "" "if num-anonymous-games > 0 [ plot anonymous-score / (num-anonymous-games) ]"
...
@#$#@#$#@
<experiments>
  <experiment name="tournament-mean-payoff" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>joss-score / num-joss-games</metric>
    <metric>anonymous-score / num-anonymous-games</metric>
    <steppedValueSet variable="num-strategy-agents" first="10" step="10" last="100"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
